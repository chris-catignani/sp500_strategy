...
class TestDerivedConstituentSeries(unittest.TestCase):
    ...
    def test_the_annual_series_holds_every_december_the_quarterly_one_does(self):
        """A fourth-quarter observation and an annual one are the same reading.

        Both are December 31 prices taken from the same audited roster, so a ticker the
        quarterly series observes at `YYYY-Q4` must appear in the annual series at `YYYY`
        at the same price. The two files are regenerated by different scripts, which is
        how they come apart: #112 regenerated the quarterly series when it had fallen
        behind its own inputs and left the annual one alone, so EMC and Viacom kept
        fourth-quarter observations with no annual twin for 2014 onward.

        That gap reached a figure. EMC's terminal action values its DVMT stock leg at the
        last price the series observes, and the quarterly side moved to 2015 while the
        annual side stayed on 2006 -- one event carrying two prices that differ by about
        a factor of two. Nothing caught it, because each file was internally consistent
        and every existing check reads one file at a time.
        """
        quarterly_path = (
            ROOT / "data" / "raw" / "ground_truth" / "derived_quarterly_constituent_series.json"
        )
        with open(quarterly_path, "r", encoding="utf-8") as f:
            quarterly = json.load(f)["series_by_ticker"]

        compared = 0
        for ticker, observations in quarterly.items():
            for period, observation in observations.items():
                if not period.endswith("-Q4"):
                    continue
                year = period.split("-")[0]
                annual = self.series.get(ticker, {}).get(year)
                with self.subTest(ticker=ticker, period=period):
                    self.assertIsNotNone(
                        annual,
                        f"{ticker}: the quarterly series holds {period} but the annual "
                        f"series holds no {year} -- one of the two is behind its inputs",
                    )
                    self.assertEqual(
                        annual["price_usd"],
                        observation["price_usd"],
                        f"{ticker} {year}: the two series disagree on a December 31 price",
                    )
                compared += 1
        self.assertGreater(compared, 100, "the annual/quarterly cross-check degenerated")
