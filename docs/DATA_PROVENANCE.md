...
**What it did to the published results.** The quarterly Top 5 and Top 10 thirty-year figures fall, because the correction removes wealth that was being counted twice; the annual path does not move at all, since its one endpoint was already reconciled. No magnitude is printed here: a number describing how a data change moved portfolio performance is the case AGENTS.md names as failing the gate outright, because the reader needs the direction and not the size. Run `python3 run_backtest.py --frequency quarterly --no-export` for the current levels.
