...
├── ground_truth/
│   ├── quarterly_ground_truth_holdings.json # Audited SEC EDGAR Form N-30D / NPORT-P holdings (1995–2024, 35 verified quarters)
│   ├── sec_filings/                         # Archive of 47 filings (20 NPORT-P XML, 26 historical annual reports, SPY FY2004 N-30D)
│   └── universe_gap_report.json             # Historical survivorship gap report (36 missing constituents at depth 30)
├── tickers/
...
- **SPDR S&P 500 ETF Trust (SPY)**: While SPY launched in January 1993, public historical portfolio snapshots prior to 2020 were filed only annually on Form N-30D (later Form N-CSR) for SPY's fiscal year-end (September 30). SPY has never filed Form N-Q quarterly reports, so there is no primary regulatory filing for pre-2020 calendar quarter-ends (March 31, June 30, or December 31) in this filer. Quarterly point-in-time filings began only with Form NPORT-P XML in 2020. To provide verifiable ground truth without external dependencies, the repository archives 47 primary SEC EDGAR filings (20 NPORT-P XML, 26 historical annual reports, plus the newly archived SPY FY2004 N-30D) and parses them via a pure Python standard-library parser (§4.3.6) to establish 35 verified quarters and quantify the historical survivorship gap.
...
To eliminate reliance on third-party aggregators and establish regulatory ground truth, **47 primary SEC EDGAR regulatory filings** of the SPDR S&P 500 ETF Trust (`SPY`, CIK `0000884394`) are permanently archived in `data/raw/ground_truth/sec_filings/`:
- **Coverage Scope (1995–2024)**:
  ...
  - **Historical Annual Reports (1995–2019, 26 Filings + 1 FY2004 Replacement)**: Form `N-CSR` and `N-30D` filings containing the complete audited **Schedule of Investments**. **15 of these historical filings are extracted** (fiscal years 1995–2009, each a September 30 snapshot). The remaining 10 annual reports (2010–2019) are HTML-era filings tracked under issue #40.
  - **2004 Archive Correction**: Accession `0000950135-04-005558` (Form `N-CSR`, filed 2004-12-03) was previously archived as the FY2004 report. When strict total validation was implemented, the parser detected multiple schedule totals across nine distinct series (the Select Sector SPDR Trust) and found no SPY schedule in the document. The filing is retained in the archive for provenance auditability with `"spy_schedule": false` and is not a SPY source; the authentic SPY FY2004 Form `N-30D` annual report (accession `0000950135-05-000037`, filed 2005-01-05; amended by N-30D/A `0000950135-05-000099` with an identical schedule) was archived to replace it, parsing to a dollar-exact match against the filing's stated \$43,213,831,431 portfolio total.
  - **Verified Quarters**: Verified quarters rise from **23 to 35** (20 modern NPORT-P quarters + 15 historical Form N-30D Q3 snapshots). The 30 historical Q1/Q2 calendar quarters (plus 1994) lack regulatory filings in this filer and are labeled `[UNVERIFIED - No Filing]`.
- **Automated Standard-Library Parser (`scripts/extract_ground_truth_from_sec.py`)**:
  ...
  - `parse_n30d_filing()` parses the fixed-width **Schedule of Investments** across all 15 historical Form N-30D annual reports using standard library regex, joining company names wrapped across continuation lines and consolidating multiple positions in the same issuer.
  ...
  - Programmatically generates [`data/raw/ground_truth/quarterly_ground_truth_holdings.json`](../data/raw/ground_truth/quarterly_ground_truth_holdings.json) with verification metadata, per-holding weights, and the fund's total portfolio value. All 15 historical extractions and 20 XML extractions are asserted by `tests/test_raw_constituents.py::TestN30DScheduleParser`.
- **Year-End Estimation Invariant**:
  - **The year-end (December 31) rosters remain unvalidated for 1994–2019.** SPY's fiscal year ends September 30 and SPY has never filed Form N-Q, so there is no December 31 primary source in this filer for any pre-2020 year. These 15 filings raise validation coverage and enumerate the survivorship gap; they do not replace the 208 `Unverified Estimate` rows.
- **Reconciliation Accuracy**:
  - Across all **35 verified regulatory filing quarters** (20 XML + 15 N-30D Annual Reports), average Top 10 match accuracy is **91.4%** (320/350).
  - Across the **15 historical Form N-30D quarters (1995–2009)**, accuracy is **84.0%** (126/150).
  ...
  - **Circular-Q4 Caveat**: The five modern Q4 filings (2020-Q4 … 2024-Q4) match 10/10 by construction because the year-end candidate lists are themselves parsed from those exact filings. `scripts/audit_quarterly_expansion.py` reports both figures and `CIRCULAR_Q4_PERIODS` names the excluded quarters.
- **Historical Universe Gap & Survivorship Bias Analysis**:
  - The 15 extracted filings reveal a persistent historical universe gap: constituents appearing in the filings' Top 30 that have no price series in `data/raw/tickers/`.
  - An exhaustive gap audit is published at [`data/raw/ground_truth/universe_gap_report.json`](../data/raw/ground_truth/universe_gap_report.json), enumerating **36 missing constituents** across the 1995–2009 filings (depth 30).
  - Four missing constituents reached the Top 10 in audited filings:
    ...
    - `LU` (Lucent Technologies Inc., #7 peak rank, present in 4 filings, Top 10 in 1997–2000)
