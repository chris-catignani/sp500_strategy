...
- **Pricing & Dividends**: Vendor-sourced prices (`data/sp500_prices.json`, `data/sp500_quarterly_prices.json`) and dividends per share (`data/sp500_dividends.json`, `data/sp500_quarterly_dividends.json`) are split-adjusted to 2024-12-31. The constituents priced from fund schedules are the exception at **both** ends: their prices are adjusted to each registrant's **own final observation** rather than to the present (a delisted registrant has no subsequent corporate actions, so adjustment to today is undefined), and their dividends are converted into that same basis by `scripts/derive_issuer_dividend_series.py`. A dividend and a price for one of these constituents are comparable to each other and **not** to `data/raw/tickers/`. See `docs/DATA_PROVENANCE.md` §4.1, §4.3.12 and §4.3.15.
...
- **Published Figures Must Be Sourced or Asserted**: a figure in `docs/` or `README.md` is one of two things. **Sourced** — read from a filing and carrying an accession — cannot rot, because the filing cannot change; `scripts/verify_provenance.py` re-checks these. **Computed** — anything the code produces, from row counts to CAGRs — rots on any data or engine change, and may be published *only if a test asserts it*. If you will not assert it, state the claim qualitatively and point at the script that prints the magnitude. Assert the **claim, not the cell value**: pin invariants exactly (a market-cap book cannot see past the pool cut), signs and thresholds with headroom (momentum at Top 10 is negative in all six cells; the survivorship effect stays under 0.5pp). Pinning every published value relocates the maintenance chore instead of removing it — a legitimate data improvement should move a table, not fail twenty assertions. Tables of computed figures carry the command that regenerates them. This extends to computed figures the discipline §4.3.13 already applies to sourced ones: a number nothing checks is an assertion, not a fact.
