...
> - **Quarterly Rebalancing**: At the end of each quarter (March 31, June 30, September 30, December 31), exact split-adjusted dividends paid across that 3-month window are credited to cash prior to rebalancing. This captures authentic intra-year dividend increases (e.g., Apple, Microsoft, ExxonMobil dividend raises) in the exact quarter they took effect. Candidate pools for Q1–Q3 dynamically drift the prior December's **Top 20** point-in-time factsheet constituents relative to index performance, re-anchoring to official factsheet weights at Q4. Ranks #1–#12 use S&P Dow Jones year-end factsheet weights, 2020–2024 candidates derive directly from SPY's audited Form NPORT-P XML filings, and ranks #13–#20 for all other years are **unverified estimates** — no primary source in this repository reports a capitalization for them, and each such row is labelled accordingly in [`docs/historical_weights_table.csv`](docs/historical_weights_table.csv) (208 of 620 rows). Against the archived `SPY` filings in `data/raw/ground_truth/sec_filings/`, the drift model reaches **96.0% out-of-sample Top 10 accuracy** across the 15 NPORT-P quarters that are genuine tests. The headline 97.0% figure across all 20 NPORT-P quarters is inflated: the five Q4 filings are the same documents the year-end candidate lists are parsed from, so they match 10/10 by construction. Across all 23 verified filings (including the three Form N-30D annual reports) accuracy is 95.2%. Of the 24 mid-year promotions from ranks #13–#20 that the expansion enables, only **7 are corroborated by an audited filing** (Oracle 2000, Citigroup 1999, NVIDIA 2021, Tesla 2023); 16 fall in quarters with no archived filing, and 1 — Walmart in 2008-Q3 — is contradicted by the filing, which places it at #11.
> - **Selector Dynamics under Top 20 Expansion**: Two separate effects are easy to conflate. *Pool size alone* (holding the constituent data fixed, per `scripts/audit_quarterly_expansion.py`): for `MarketCapSelector`, Top 3 and Top 5 are unchanged at 0.00% delta — a name ranked #13–#20 cannot enter a Top 5 book — while Top 10 gains modestly (+0.24% 10y, +0.59% 20y, +0.05% 30y pre-tax). For `PerformanceSelector` the expansion is **negative in 8 of 9 strategy/horizon cells**, averaging **-2.2%** and reaching **-5.89%** (Top 10, 10y); the single positive cell is Top 3 at 10y (+2.92%). A wider pool gives a momentum selector more recent winners to chase, and buying the largest recent gainer out of 20 large caps runs into mean reversion more often than out of 12. *Separately*, correcting the 2021–2023 year-end weights against the NPORT-P filings changed the underlying data and therefore moved the reported results: Top 3 Market Cap fell from 26.88% to 24.49% (10y) and 15.16% to 14.43% (30y), because the prior data ranked NVIDIA #3 at year-end 2023 where the filing shows it #4–#5 behind Alphabet.
> - **Issuer-Level Consolidation & Dual-Class Execution**: Multi-class share structures are consolidated at the corporate issuer level based on combined market capitalization, mapping trade execution to the primary liquid voting share class (Alphabet Class A `GOOGL`). Read as filed in SPY's Form NPORT-P filings, Alphabet is split into two positions (Class A `02079K305` and Class C `02079K107`), and the S&P 500 index ranks each share class independently. At year-end 2023, the raw filing reports AAPL 7.03%, MSFT 6.98%, AMZN 3.45%, NVDA 3.06%, Alphabet A 2.07%, META 1.96%, and Alphabet C 1.75%. Aggregating Alphabet's share classes establishes a combined 3.82% enterprise weight ranking #3, placing Alphabet in the 2024 Top 3 portfolio; unconsolidated as-filed rankings yield AAPL/MSFT/AMZN. Holding both classes in Top 10 books consumes two slots and creates portfolio degeneracy. **The consolidation runs only where the source reports each class separately** — the 2020-2024 NPORT-P filings. Alphabet's 2014-2019 year-end weights come from hand-entered factsheet anchors with no archived December-dated filing behind them, and they may understate the consolidated issuer weight; those rows are marked unverified in [`docs/historical_weights_table.csv`](docs/historical_weights_table.csv) and the gap is tracked in issue #45. For the full filing breakdown and execution convention rationale, see Section 4.3.8 in [`docs/DATA_PROVENANCE.md`](docs/DATA_PROVENANCE.md#438-dual-class-issuer-consolidation--execution-convention).
