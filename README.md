...
> - **Quarterly Rebalancing**: At the end of each quarter (March 31, June 30, September 30, December 31), exact split-adjusted dividends paid across that 3-month window are credited to cash prior to rebalancing. This captures authentic intra-year dividend increases (e.g., Apple, Microsoft, ExxonMobil dividend raises) in the exact quarter they took effect. Candidate pools for Q1–Q3 dynamically drift the prior December's **Top 20** point-in-time factsheet constituents relative to index performance, re-anchoring to official factsheet weights at Q4. Ranks #1–#12 use S&P Dow Jones year-end factsheet weights, 2020–2023 candidates derive directly from SPY's audited Form NPORT-P XML filings, and ranks #13–#20 for all other years are **unverified estimates** — no primary source in this repository reports a capitalization for them, and each such row is labelled accordingly in [`docs/historical_weights_table.csv`](docs/historical_weights_table.csv) (216 of 620 rows). Against the archived `SPY` filings in `data/raw/ground_truth/sec_filings/`, the drift model reaches **95.7% out-of-sample Top 10 accuracy** across the 14 NPORT-P quarters that are genuine tests. The headline 96.7% figure across all 18 NPORT-P quarters is inflated: the four Q4 filings are the same documents the year-end candidate lists are parsed from, so they match 10/10 by construction. Across all 21 verified filings (including the three Form N-30D annual reports) accuracy is 94.8%. Of the 24 mid-year promotions from ranks #13–#20 that the expansion enables, only **7 are corroborated by an audited filing** (Oracle 2000, Citigroup 1999, NVIDIA 2021, Tesla 2023); 16 fall in quarters with no archived filing, and 1 — Walmart in 2008-Q3 — is contradicted by the filing, which places it at #11.
...
> - **Alphabet share-class aggregation is a load-bearing assumption**: SPY's filings carry Alphabet as two positions (Class A `02079K305`, Class C `02079K107`), and the S&P 500 ranks them as two separate constituents. This project consolidates them into a single `GOOGL` position, which moves Alphabet from rank ~5 to rank 3 at year-end 2023 and so determines the entire 2024 Top 3 book. Read as filed, the 2023 Top 3 would be AAPL/MSFT/AMZN instead of AAPL/MSFT/GOOGL.
