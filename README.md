...
> - **Selector Dynamics under Top 20 Expansion (issue #41, decided)**: Two separate effects are easy to conflate. *Pool size alone* (holding the constituent data fixed, per `scripts/audit_quarterly_expansion.py`): `MarketCapSelector` is never hurt — Top 3 and Top 5 measure 0.00% delta at every horizon on both paths, and Top 10 gains (+0.24% 10y, +0.70% 20y, +0.57% 30y pre-tax, quarterly). `PerformanceSelector` is **negative in 7 of 9 quarterly cells**, averaging **-1.64%** and reaching **-5.89%** (Top 10, 10y), because a wider pool gives a momentum selector more recent winners to chase and the largest recent gainer out of 20 mean-reverts more often than out of 12. **The pool stays uniform at 20**: the sign of the momentum effect flips with N and with rebalancing frequency, so a rule fitting it would need pool size to depend on selector x N x frequency — twelve parameters on one 31-year sample — while the shipped market-cap default is hurt nowhere. The asymmetry is documented rather than tuned; see `docs/DATA_PROVENANCE.md` §4.3.7. *Separately*, correcting the 2021–2023 year-end weights against the NPORT-P filings changed the underlying data and therefore moved the reported results: Top 3 Market Cap fell from 26.88% to 24.49% (10y) and 15.16% to 14.43% (30y), because the prior data ranked NVIDIA #3 at year-end 2023 where the filing shows it behind Alphabet; the 30y level has since moved again to 13.77% as survivorship was fixed and the derived constituents' dividends were sourced (§4.3.18).
